<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:74:"D:\wamp64\www\shemblog\public/../application/index\view\article_index.html";i:1532133087;s:67:"D:\wamp64\www\shemblog\public/../application/index\view\layout.html";i:1532508523;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="utf-8">
...
<ol class="breadcrumb sider-margin">
    ...
    <li class="active">学海无涯&nbsp;|&nbsp;<?php echo $articledata['menu_name']; ?></li>
</ol>
...
<div class="articleinfo">
    <h5>
        <small>[ <?php if($articledata['art_original'] == '1'): ?>原创<?php else: ?>转载<?php endif; ?> ]</small>
        <a><?php echo $articledata['art_title']; ?></a>
    </h5>
    <div class="articleinfo-write">栏目：<?php echo $articledata['menu_name']; ?>&nbsp;&nbsp;作者：<?php echo $articledata['art_author']; ?>&nbsp;&nbsp;阅读：（<?php echo $articledata['art_hit']; ?>）<span class="hidden-xs">更新于：<?php echo getTime($articledata['art_addtime']); ?></span></div>
    <div class="articleinfo-content"><?php echo $articledata['art_content']; ?></div>
    <div class="articleinfo-copy hidden-xs">
        <?php if($articledata['art_original'] == '1'): ?>
            <p>非特殊说明，本文版权归 <?php echo \think\Config::get('auth.blogname'); ?> 所有，转载请注明出处.</p>
            <p>本文标题：<?php echo $articledata['art_title']; ?></p>
            <p>本文网址：<a class="url"><script>$('.url').append(window.location.href);</script></a>.</p>
            <p>本文标签：<a><?php echo $articledata['art_keyword']; ?></a></p>
            <?php else: ?>
    ...
    <div class="articleinfo-up">
        <p>上一篇：
            <?php if(empty($other['up']) || ($other['up'] instanceof \think\Collection && $other['up']->isEmpty())): ?>
            &nbsp;没有了
            <?php else: ?>
            <a title='<?php echo $other['up']['art_title']; ?>' href="<?php echo url('Article/index',['id'=>$other['up']['art_id']]); ?>">&nbsp;<?php echo msubstr($other['up']['art_title'],0,40,'utf-8',true); ?></a>
            <?php endif; ?>
        </p>
    </div>
    <div class="articleinfo-down">
        <p>下一篇：
            <?php if(empty($other['down']) || ($other['down'] instanceof \think\Collection && $other['down']->isEmpty())): ?>
            &nbsp;没有了
            <?php else: ?>
            <a title='<?php echo $other['down']['art_title']; ?>' href="<?php echo url('Article/index',['id'=>$other['down']['art_id']]); ?>">&nbsp;<?php echo msubstr($other['down']['art_title'],0,40,'utf-8',true); ?></a>
            <?php endif; ?>
        </p>
    </div>
    <hr />
    <?php if(is_array($articlecommondata) || $articlecommondata instanceof \think\Collection): $i = 0; $__LIST__ = $articlecommondata;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$artcom): $mod = ($i % 2 );++$i;?>
        <div class="media connect" id="<?php echo $artcom['com_id']; ?>">
            <div class="media-left">
                <img src="<?php echo $artcom['mem_img']; ?>" class="media-object img-circle img-50" alt="<?php echo $artcom['mem_name']; ?>"/>
            </div>
            <div class="media-body">
                <div class="fool hidden-xs">#<?php echo $artcom['com_id']; ?></div>
                <p class="media-heading">
                    <?php echo $artcom['mem_name']; if($artcom['mem_sex'] == '1'): ?>
                    <i class="iconfont icon-boy"></i>
                    <?php else: ?>
                    <i class="iconfont icon-girl"></i>
                    <?php endif; ?>
                    <i class="iconfont icon-time"></i>&nbsp;<?php echo getTime($artcom['com_addtime']); ?>&nbsp;&nbsp;
                    <a class="from"><?php echo $artcom['com_from']; ?></a>&nbsp;&nbsp;
                    <i class="iconfont icon-address"></i>&nbsp;
                    <?php echo $artcom['com_city']; ?>
                </p>
                <div class="connect-box"><?php echo $artcom['com_content']; ?></div>
                <?php if($artcom['com_status'] == '1'): ?>
                    <hr />
                    <div class="media">
                        <div class="media-left">
                        ...
                        <div class="media-body">
                            <p class="media-heading">
                                ...
                                <i class="iconfont icon-time"></i>&nbsp;<?php echo date('m/d H:i',$artcom['com_rtime']); ?>&nbsp;&nbsp;&nbsp;&nbsp;回复&nbsp;
                                <a>@<?php echo $artcom['mem_name']; ?></a>&nbsp;中说到：
                            </p>
                            <div class="connect-box"><?php echo $artcom['com_rcontent']; ?></div>
                        </div>
                    </div>
                <?php endif; ?>
            </div>
        </div>
    <?php endforeach; endif; else: echo "" ;endif; if(count($articlecommondata) >= 5): ?>
    <div class="more">
        <span class="loadingmore" data-id="<?php echo \think\Request::instance()->param('id'); ?>" data-lenth="1" data-url="<?php echo url('Article/ajaxList'); ?>">加载更多</span>
    </div>
    <?php endif; ?>
    <div class="well">
        ...
        <button class="btn btn-default btn-add" type="button" data-action="<?php echo url('Comment/add'); ?>" data-artid="<?php echo $articledata['art_id']; ?>">Ctrl + Enter 快速提交</button>
    </div>
</div>
<script src="home_js/jquery-1.10.1.min.js"></script>
